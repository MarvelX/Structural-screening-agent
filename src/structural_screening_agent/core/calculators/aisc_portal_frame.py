...
class AISCPortalFrameCalculator:
    ...
    def calculate(self, case: PortalFrameScreeningCase) -> PortalFrameScreeningResult:
        ...
        return PortalFrameScreeningResult(
            ...
            code_path="aisc",
            calculation_summary=(
                "AISC portal-frame first-pass screening based on purlin tributary load plus primary-frame rafter/column added-moment proxies."
            ),
            calculation_rows=rows,
            controlling_component=controlling_component,
            controlling_path=controlling_path,
            code_reference_ids=["aisc_portal_frame_purlin_screening"],
        )
